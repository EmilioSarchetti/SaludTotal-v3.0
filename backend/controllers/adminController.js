...
exports.registrarMedico = (req, res) => {
    const { nombre, apellido, email, contraseña, especialidades, horarios } = req.body;

    const sqlUsuario = `
        INSERT INTO usuarios (nombre, apellido, email, contraseña, tipo)
        VALUES (?, ?, ?, ?, 'medico')
    `;

    db.query(sqlUsuario, [nombre, apellido, email, contraseña], (err, resultado) => {
        if (err) {
            console.error(err);
        ...
        db.query(sqlEspecialidad, [valoresEspecialidad], (err) => {
            ...
            const sqlHorario = `INSERT INTO horarios_medicos (medico_id, dia_semana, hora_inicio, hora_fin) VALUES ?`;
